...
type_suffix_pattern = re.compile('''\\s*\\(?(?:\\s*shav(?:ing|e)\\s+(?:soap|cream|puck)
        |\\s*(?:soap\\s*|)sampler?
        |\\s*(?:soap|cream)(?:\\s*(?:\\/|&|\\+|,|\\s)\\s*(?:splash|post)|)
...
_fougere = 'four?g[èeé]re'
_sandalwood = 'sand(?:al|le) *wood'
_no_dot = '(?:number|n^?o\\.?|#)'
_barbershop = 'bar?ber(?: *shop|)'
...
letterlead = re.compile('\w')

def comp_pattern( base_pattern ):
    _prefix = '\\b'
    _suffix = '\\b[\\.,]?'
    if not letterlead.match(base_pattern):
        _prefix = ''
    if base_pattern[-1] == '$':
        _suffix = ''
    return re.compile(_prefix + base_pattern + _suffix, re.IGNORECASE)
    ...
    def _compile( self, lowpatterns: dict, highpatterns: dict, bases: list ):
        ...
        if lowpatterns:
            for pat in lowpatterns:
                self.lowpatterns[comp_pattern(pat)] = lowpatterns[pat]
                if lowpatterns[pat] not in allnames:
                    allnames.append(lowpatterns[pat])
        if highpatterns:
            for pat in highpatterns:
                finalpat = pat
                self.highpatterns[comp_pattern(finalpat)] = highpatterns[pat]
    ...
    def strip_base( self, text: str ):
        ...
        for pattern in self.bases:
            ...
            if stripped or (stripped == '' and self.default_scent):
                result = separator_pattern.match(stripped)
                if result:
...
def custom_ariana_evans( map ):
    if map and map['scent'] == 'Passiflora':
...
def custom_extro( map ):
    if map:
        if map['scent'] == 'La Prima':
            map['maker'] = 'NassRasur Club'
    return map

def custom_maeurer_wirtz( map ):
    if map:
        if "irisch" in map['scent'].lower() or "irish mo" in map['scent'].lower():
            map['maker'] = 'Sir Irisch Moos'
            map['scent'] = 'Sir Irisch Moos'
    return map


_default_sniffer = Sniffer()
...
_scent_pats = {
    ...
    'Angora Soaps': Sniffer(
        lowpatterns={
            'high *roller': 'High Roller',
            f'{_barbershop}': 'Barbershop',
        }
    ),
    ...
    'Ariana & Evans': Sniffer(
        patterns={
            ...
            f'which one(?:{_apostrophe}| *i)s pink\\??': 'Which One\'s Pink?',
        ...
        lowpatterns={
            ...
            't[ao]bac+o? de vanil+[ea]': 'Vanille de Tabac',
        },
    ...
    'Asylum Shave Works': Sniffer(
        patterns={
            'levante': 'Levante',
            'flying mango': 'Flying Mango',
        },
        lowpatterns={
            'aloe': 'Aloe',
            'bay rum': 'Bay Rum',
            'bergamot': 'Bergamot',
            'charcoal': 'Charcoal',
            '(?:european *|)barbershop': 'European Barbershop',
        },
        bases=[ 'english collection' ]
    ),
    ...
    'Australian Private Reserve': Sniffer(
        ...
        lowpatterns={
            ...
            f'g{_apostrophe}day,? mate': 'G\'Day Mate',
        },
        custom=custom_apr
    ),
    ...
    'Barbasol': Sniffer(
        ...
        lowpatterns={
            '(?:original|classic)': 'Original', # "thick and rich"
            'sensitive': 'Sensitive Skin', # blue
            'soothing aloe': 'Soothing Aloe', # green
            'extra moisturizing': 'Extra Moisturizing',
            '(?:100th anniversary|1919)': '100th Anniversary',
            'purple': 'Extra Moisturizing',
        },
    ...
    'Barrister and Mann': Sniffer(
        patterns={
            ...
            'm[éeè]lange': 'Mélange',
            f'figgy puddin(?:{_apostrophe}|g)': "Figgy Puddin'",
            'ja[cs]inthe?': 'Jacinthe',
            'amaz(?:el|le)nut': 'Amazelnut',
            'citr[iu]c[ua]y[áaà]': 'Citricuyá',
            'figaros[ea]': 'Figarose',
            'vespers?': 'Vespers',
            f'vetiver,? *heather,?{_any_and}clary sage': 'Vetiver, Heather, & Clary Sage',
            # was initially a club release
    ...
    'Baxter of California': Sniffer(
        lowpatterns={
            'super close shave formula': 'Super Close Shave Formula',
            'beard line[\\- ]*up (?:shave |)gel': 'Beard Line-up Shave Gel',
        }
    ),
    ...
    'Catie\'s Bubbles': Sniffer(
        patterns={
            ...
            '322': '322',
            f'{_apostrophe}95 liquid labels': '\'95 Liquid Labels'
        },
        lowpatterns={
    ...
    'CBL Soaps': Sniffer(
        ...
        lowpatterns={
            ...
            'outlaw': 'Outlaw',
            'grape *soda': 'Grape Soda', # maybe doesn't matter; u/PhilosphicalZombie may be the only one to have this scent
        },
    ...
    'Cella': Sniffer(
        lowpatterns={
            ...
            '(?:cella |milano |)cre(?:ma|am) d[ae] barba': 'Cella',
    ...
    'Central Texas Soaps': Sniffer(
        ...
        lowpatterns={
            f'gra(?:nd|m)pa{_apostrophe}s *{_barbershop}': 'Grandpa\'s Barbershop',
            'saw': 'The Saw',
        }
    ),
    ...
    'Chicago Grooming Co.': Sniffer(
        patterns={
            ...
            'armon[íiì]a': 'Armonía',
            f'windy city {_barbershop}': 'Windy City Barbershop',
        },
        lowpatterns={
            ...
            '"?darkwing"?(?: tester|)$': 'Unknown',  # new base testers
        },
        bases=[ 'canard', 'dark *wing' ]
    ),
    ...
    'Chiseled Face': Sniffer(
        patterns={
            ...
            'gem[üu]tlichkeit': 'Gemütlichkeit',
        },
        lowpatterns={
    ...
    'Cremo': Sniffer(
        lowpatterns={
            ...
            'sandalwood': 'Sandalwood',
            f'sage{_any_and}citrus': 'Sage & Citrus',
        }
    ),
    ...
    'Declaration Grooming': Sniffer(
        ...
        lowpatterns={
            ...
            f"catalan{_apostrophe}s prai?rie": "Catalan's Prairie",
            'dfs (?:2017|exclusive)': 'DFS 2017 LE',
            'sol': 'Sunrise on LaSalle',
            'puzzle$': 'Puzzle (2016)',
        },
        bases=[ 'bison', 'milksteak', 'icarus' ]
    ),

    'Dick Johnson': Sniffer(
        lowpatterns={
            '(?:roasted|smoke?[dy]) licorice': 'Roasted Licorice',
            'sweet rum': 'Sweet Rum',
            'whiske?y' + _any_and + 'vanilla': 'Whiskey & Vanilla',
        },
        bases=['super mousse', 'm[oe}{1,2}ll[eu]{1,2}x?']
    ),
    ...
    'Epsilon': Sniffer(
        lowpatterns={
            '(?:blue |)mediterranean': 'Blue Mediterranean',
            'scottish spirit': 'Scottish Spirit',
            'amber': 'Amber',
        }
    ),
    ...
    'Extrò Cosmesi': Sniffer(
        ...
        lowpatterns={
            ...
            'bay rum': 'Bay Rum',
            'dandy': 'Dandy',
            'la prima': 'La Prima', # NassRasur Club
        },
        custom=custom_extro
    ),
    ...
    'EZ Shave': Sniffer(
        lowpatterns={
            'shav(?:ing|e) gel': 'Shaving Gel',
            'shav(?:ing|e) cream': 'Shaving Cream',
        }
    ),
    ...
    'Fine Accoutrements': Sniffer(
        ...
        lowpatterns={
            f'l?{_apostrophe}orange *noir': "l'Orange Noir",
    ...
    'Gentlemans Face Care Club': Sniffer(
        lowpatterns={
            'peppermint': 'Peppermint',
            f'bergamot{_any_and}neroli': 'Bergamot & Neroli',
            'carribean lime': 'Carribean Lime',
            _sandalwood: 'Sandalwood',
        }
    ),
    ...
    'Godrej': Sniffer(
        lowpatterns={
            ...
            '(?:godrej |)shaving round': 'Shaving Round',
        }
    ),
    ...
    'Grooming Department': Sniffer(
        patterns={
            ...
            'l' + _apostrophe + 'av+entur+a': 'l\'Avventura',
            ...
            f'jasmine{_any_and}civet': 'Jasmine & Civet',
        },
        lowpatterns={
            'Aion': 'Aion',
            'Amare': 'Amare',
    ...
    'Hampshire Woolfat': Sniffer(
        lowpatterns={
            'no\\.? *3': 'No. 3',
            f'may chang{_any_and}lavender': 'May Chang & Lavender',
        }
    ),
    ...
    'Henri et Victoria': Sniffer(
        patterns={
            'beyond the void(?:: gadabout|)$': 'Beyond the Void: Gadabout',
        },
        lowpatterns={
            'havana breeze': 'Havana Breeze',
        }
    ),
    ...
    'House of Mammoth': Sniffer(
        patterns={
            ...
            'santa[^\W\s]+ noir[\u274ce]?': 'Santa Noir',
            'santa(?:~~l~~|) noir': 'Santa Noir',
            'f[úuù] d[àaá]o(?: \u798f|)': 'Fú Dào',
            '\u798f(?: \\(?f[úuù] d[àaá]o\\)?|)': 'Fú Dào',
        },
        lowpatterns={
            'cirrus': 'Cirrus',
    ...
    'Sir Irisch Moos': Sniffer(
        default_scent='Sir Irisch Moos'
    ),
    ...
    'Marlowe': Sniffer(
        lowpatterns={
            '(?:no\\.|#) *141': 'No. 141',
            '(?:no\\.|#) *142': 'No. 142',
            'shave cream (?:with|w\\/?) willow bark extract': 'No. 141',
            'shave gel (?:with|w\\/?) willow bark extract': 'No. 142',
        }
    ),

    'Marmara Barber': Sniffer(
        lowpatterns={
            '(?:n\\^?o\\.?|#) 1': 'No. 1',
            '(?:n\\^?o\\.?|#) 2': 'No. 2',
            '(?:n\\^?o\\.?|#) 3': 'No. 3',
    ...
    'Murphy and McNeil': Sniffer(
        patterns={
            ...
            'gael la?och orange': 'Gael Laoch Orange',
            'gael la?och': 'Gael Laoch',
            'ouroboros': 'Ouroboros',
        },
        lowpatterns={
        ...
        bases=[ 'sl[aá]inte', 'aon', 'kodiak',
                'infinitus' ] # not a base, but class of scent release
    ),
    ...
    'NassRasur Club': Sniffer(
        lowpatterns={
            'la prima': 'La Prima', # Extrò Cosmesi
            'patronus': 'Patronus', # Goodfellas smile
        },
    ),
    ...
    'Oz Shaving Co.': Sniffer(
        patterns={
            '(?:40|fou?rty) *crows': '40 Crows',
        },
        lowpatterns={
            'tic?k *toc?k': 'Tick Tock',
        }
    ),
    ...
    'RazoRock': Sniffer(
        patterns={
            '[\W]*what the puck[\W]*\\s+blue': 'Blue Barbershop',
            '[\W]*what the puck[\W]*\\s+lime(?: burst|)': 'What the Puck?! Lime Burst',
            '[\W]*what the puck[\W]*\\s+black': 'What the Puck?! Black Label',
            '[\W]*what the puck[\W]*\\s+gold': 'What the Puck?! Gold Label',
            '[\W]*what the puck[\W]*\\s+green': 'What the Puck?! Green Label',
    ...
    'Red House Farm': Sniffer(
        patterns={
            ...
            f'(?:9|nine) days up nort{_apostrophe}h?': '9 Days up Nort\'',
        },
        lowpatterns={
    ...
    'Rituals': Sniffer(
        lowpatterns={
            # the ritual of samurai is apparently their only shave line
            '(?:the |)ritu[ae]l of samurai': 'The Ritual of Samurai',
            'shav(?:ing|e) (?:cream|gel|foam)': 'The Ritual of Samurai',
            'samurai': 'The Ritual of Samurai',
        },
        default_scent='The Ritual of Samurai',
    ),
    ...
    'Squadron': Sniffer(
        patterns={
            f'group captain{_apostrophe}s blend': 'Group Captain\'s Blend',
            'red lead': 'Red Lead',
            'six one seven': 'Six One Seven',
        },
        lowpatterns={
            '6-?1-?7': 'Six One Seven',
        }
    ...
    'Stirling Soap Co.': Sniffer(
        patterns={
            ...
            f'(?:scot+{_apostrophe}s|scotch|) pine sheep': 'Scots Pine Sheep',
    ...
    'Summer Break Soaps': Sniffer(
        patterns={
            ...
            'mountain laurel': 'Mountain Laurel',
            '(?:lg |)roty 2020': 'ROTY 2020', # K1986
            '(?:lg |)2020 roty': 'ROTY 2020',
            '(?:lg |)roty 2021': 'ROTY 2021', # jwoods23
            '(?:lg |)2021 roty': 'ROTY 2021',
        },
        lowpatterns={
            'recess': 'Recess',
    ...
    '345 Soap Co.': Sniffer(
        lowpatterns={
            ...
            'rhumpkin': 'Rhumpkin',
            f'skinny dippin(?:{_apostrophe}|g)': "Skinny Dippin'",
            "fresh cut fade": "Fresh Cut Fade",
            f"ol{_apostrophe} *red": "Ol' Red",
        }
    ),
    ...
    'Valobra': Sniffer(
        ...
        bases=[ 'soft shaving soap', 'soft', 'hard' ]
    ),
...
def _compile_all():
    ...
    for maker in _scent_pats:
        if isinstance(_scent_pats[maker], Sniffer):
            ...
            isonames = { }
            for pattern in map:
                comp[comp_pattern(pattern)] = map[pattern]
...
def match_scent( maker, scent ):
    ...
    if result:
        reduced = strip_separators(scent[0:result.start()]).strip()
        if reduced:
            scent = reduced
            # repeat?
            result = type_suffix_pattern.search(scent)
            if result:
                ...
                if reduced:
                    scent = reduced
        if not reduced and so.default_scent:
            return {
                    'match': None,
                    'maker': maker,
                    'scent': so.default_scent,
                    'lather': None,
                    'search': False
                }
